...
std::vector<OptionData> processBatch(std::vector<OptionData> batch)
{
    cudaStream_t stream;
    ...
    std::vector<double> dTime(batch_size, 1.0 / 252.0);
...
int main()
{
    std::string input_filename = "/home/qhawkins/Desktop/MonteCarloCuda/timed_opra_clean_mc_small.csv";
    ...
    for (size_t i = 0; i < print_limit; ++i)
    {
        ...
        std::cout << "Underlying Price: " << result.underlying_price << std::endl;
        std::cout << "Strike Price: " << result.strike_price << std::endl;
        std::cout << "Years to Expiration: " << result.years_to_expiration << std::endl;
        std::cout << "RFR: " << result.rfr << std::endl;
        std::cout << "Option Type: " << (result.option_type == 0 ? "Call" : "Put") << std::endl;
