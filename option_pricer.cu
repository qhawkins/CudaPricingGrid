...
CRROptionPricer::CRROptionPricer(int batch_size_, double* marketPrices_, double* S_, double* K_, 
...
CRROptionPricer::~CRROptionPricer() {
...
void CRROptionPricer::retrieveOptionPrices(std::vector<double>& host_price) {
...
std::vector<Greeks> CRROptionPricer::calculateAllGreeks(const GreekParams& params, const std::vector<double>& sigma) {
    ...
    std::vector<double> reducedT(batch_size);
    for (int i = 0; i < batch_size; i++) {
        reducedT[i] = std::max(T[i] - 1.0/252.0, 0.0); // Ensure time doesn't go negative
        //std::cout << "reducedT[" << i << "]: " << reducedT[i] << std::endl;
    ...
    double* d_reducedT;
    CHECK_CUDA_ERROR(cudaMalloc((void**)&d_reducedT, batch_size * sizeof(double)));
    CHECK_CUDA_ERROR(cudaMemcpy(d_reducedT, reducedT.data(), batch_size * sizeof(double), cudaMemcpyHostToDevice));

    double* d_reducedTimePrices;
    CHECK_CUDA_ERROR(cudaMalloc((void**)&d_reducedTimePrices, batch_size * sizeof(double)));

    // Launch kernel to price options with reduced time
    int gridSize = (batch_size + 255) / 256;
    int blockSize = 256;
    priceOptionsKernel<<<gridSize, blockSize, 0, stream>>>(
        steps, batch_size, d_S, d_K, d_r, d_q, d_reducedT, d_sigma, d_optionType, d_reducedTimePrices
    );

    // Copy results back to host
    std::vector<double> reducedTimePrices(batch_size);
    CHECK_CUDA_ERROR(cudaStreamSynchronize(stream));
    CHECK_CUDA_ERROR(cudaMemcpy(reducedTimePrices.data(), d_reducedTimePrices, batch_size * sizeof(double), cudaMemcpyDeviceToHost));
    // Calculate Theta
    for (int i = 0; i < batch_size; i++) {
        greeks[i].theta = (reducedTimePrices[i] - price_original[i]);// * (1.0/252.0);
    }

    // Clean up
    CHECK_CUDA_ERROR(cudaFree(d_reducedT));
    CHECK_CUDA_ERROR(cudaFree(d_reducedTimePrices));
...
void CRROptionPricer::printGreeks(const Greeks& greeks) {
...
std::vector<double> CRROptionPricer::computeImpliedVolatilityDevice() {
